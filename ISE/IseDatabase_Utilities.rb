...
def list_jobs
  ...
  all_jobs.each_index do |x|
    puts "\t" + all_jobs[x].id.to_s + ") #{all_jobs[x].name} -- #{all_jobs[x].desc}"
  end
  puts
...
def list_models
  ...
  all_models.each_index do |x|
    puts "\t" + all_models[x].id.to_s + ") #{all_models[x].name} -- #{all_models[x].desc}"
  end
  puts
...
def list_all_jobs
  ...
  all_jobs.each_index do |x|
    printf "\t%3d) %-#{max_name_length}s  %s\n", all_jobs[x].id, all_jobs[x].name, all_jobs[x].desc
  end
...
def list_job (which_job, full = false)
  ...
  a_job_config.each_index do |x|
    unless last_model_id == a_job_config[x].model_id
      ...
      the_models[a_job_config[x].model.name] = a_job_config[x].model.desc
    end
  end
...
#  puts a_job_config.columnized
  ...
  a_job_config.each_index do |x|
    unless last_model_id == a_job_config[x].model_id
      ...
      my_rpt += "\n" + "    Desc: " + a_job_config[x].model.desc
      my_rpt += "\n" + "Location: " + a_job_config[x].model.location
      my_rpt += "\n" + "     DLL: " + a_job_config[x].model.dll
      my_rpt += "\n" + "Platform: " + a_job_config[x].model.platform.name + " -- " + a_job_config[x].model.platform.desc
