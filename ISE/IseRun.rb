#########################################################################
###
##  IseRun: Encapsulates all of the classes and methods needed to execute
...
class IseRun
  ...
  def setup
    ...
    if @state == "setup"     ## Protect against a multiple call
      ...
      begin
        ...
        ERROR(["Unhandled internal error associated with creating the guid database."])
        exit
      end
  ...
  def build_commands
    ...
    job_config.each do | jc |
      ...
      # FIXME: conflict in the way IsePeer/CrameControllerModel uses Model.dll
      #      it_is_a_ruby_model = model.dll.include?('.rb') ? true : false   ## Is this a ruby model?
      it_is_a_ruby_model = model.platform.name == 'any' ? true : false


      #if it_is_a_ruby_model
      #  the_cmd['peerd'] = ruby_peer              ## replace the IsePeer executable with the IseRubyPeer
      #end

      the_cmd  = base_cmd
      the_cmd += it_is_a_ruby_model ? ruby_peer : cpp_peer
      the_cmd += cmd_suffix
      ...
      if model.platform.name.include?('mswin32')
        the_cmd += " -c8003 -h #{$ISE_GATEWAY}"   ## 8003 is the port defined as the windows channel
        ## $ISE_GATEWAY is an IseNode running an IseDispatcher
        ## that is dedicated to the windows junk
      else
        the_cmd += " -c8001"           ## FIXME: hardcoded port number
      end
    ...
    if all_jobs.nil?
      ...
      answer = choose do |menu|
        ...
        all_jobs.each_index do |x|
          menu.choice(all_jobs[x].id.to_s + ") " +all_jobs[x].name + ": " + all_jobs[x].desc) do
