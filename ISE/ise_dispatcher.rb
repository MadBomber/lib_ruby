  ...
  def self.list (output_format=:text)
    ...
    if all_dispatchers.empty?
      ...
      all_dispatchers.each do |dispatcher|
        ...
        list_report_html_and_wiki += "|#{dispatcher.node.desc}"
        ...
        when "text" then
          ...
          list_report += "Node:#{dispatcher.node.id} #{dispatcher.node.ip_address} #{dispatcher.node.name} -- #{dispatcher.node.desc}\n"
        when "html" then
          list_report += list_report_html_and_wiki
        when "wiki" then
          list_report += list_report_html_and_wiki
