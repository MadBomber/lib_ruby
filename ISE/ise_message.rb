...
class  IseMessage
  ...
  def initialize(data=nil)    # data is a string handled by the sub-classes after super is called
    ...
    @msg_items      = []
    @raw            = ""    # SMELL: why don't we use:=> data.nil? ? "" : data
    @out            = ""
    @app_message    = nil
  ...
  def pack_message(details=false)
  ...
  ##  details is a boolean debugging flags
  def unpack_message(flags=0,details=false)
    ...
    if SimMsgFlag.test(flags, :xml_boost_serialize)
      from_xml
      return nil
    end
...
#    puts "...... format_code -=> #{rtn_array.inspect}" if $debug
  ...
  def publish
    ...
    puts "Entering IseMessage publish"  if $debug 
    $connection.publish_message self
    puts "leaving IseMessage publish"  if $debug 
...
#################################################################
  ...
  def self.subscribe(callback_method=nil, which_unit=0)
  
    puts "Entering subscribe"  if $debug  or $debug_io
    pp self if $debug or $debug_io
    $connection.subscribe_message(self, callback_method, which_unit)
