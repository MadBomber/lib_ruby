module EmFeeder
  ...
  def self.init
    ...
    $OPTIONS[:message].split(',').each do |message_name|
      Kernel.const_get(message_name).subscribe(EmFeeder.method(:feed_message))
    end
  ...
  ##       using standard callback methods based on the message name as snake case.
    ...
    StatusRequest.subscribe(  EmFeeder.method(:status_request))
    InitCase.subscribe(       EmFeeder.method(:init_case))
    EndCase.subscribe(        EmFeeder.method(:end_case))
    EndRun.subscribe(         EmFeeder.method(:end_run))
