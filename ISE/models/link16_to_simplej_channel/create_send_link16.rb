module Link16ToSimplejChannel
  ...
  def self.create_send_link16(threat_label, velocity_vector)
    ...
    if threat_label.is_missile?
      ...
      sjm_st.link16_message.identity_ = 3  if $active_tracks[threat_label].label.is_blue_force?
      sjm_st.link16_message.identity_ = 6  if $active_tracks[threat_label].label.is_red_force?
      sjm_st.link16_message.track_number_reference_ = Link16Message.encode_track_id( $active_tracks[threat_label].track_id)

      ecef_coord  = $active_tracks[threat_label].lla.to_ecef   # NOTE: ecef units in meters
      ...
      case $active_tracks[threat_label].type
      when 'UAV' then
      ...
      sjm_air.link16_message.track_number_reference_ =  Link16Message.encode_track_id( $active_tracks[threat_label].track_id)
      sjm_air.link16_message.set_lla( $active_tracks[threat_label].lla)  # TODO no velocity information ???
