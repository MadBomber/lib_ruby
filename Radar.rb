...
#    return sprintf("Min %10.6f Max %10.6f", @elevation - 5.0, @elevation + 5.0)
  ...
  def range_to(thing)
    ...
    return @range[1] + 42.69 unless lla   ## pretend its outside the maximum range
  ...
  def within_range?(thing)
    range = range_to(thing)
    puts "... range to target: #{range} radar range: #{@range.join(',')}" if $debug
     return (@range[0] <= range) && (range <= @range[1])
  end
  ...
  def within_azimuth?(thing)
    ...
    answer  = (@azimuth_min <= heading) && (heading <= @azimuth_max)
    ...
    puts "... heading to target: #{heading} radar min/max: #{@azimuth_min}/#{@azimuth_max}" if $debug
  ...
  def can_detect?(threat)
    ...
    answer = within_range?(threat)
    answer = within_azimuth?(threat) && answer
