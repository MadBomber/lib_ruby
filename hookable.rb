...
# An example of using Hookable

class MyClass
  include Hookable

  def my_method(param1, param2)
    puts "Executing my_method with params: #{param1}, #{param2}"
    "result from my_method"
  end

  # Multiple before hooks - executed in order
  before :my_method do |param1, param2|
    puts "First before hook: #{param1}, #{param2}"
    [param1 * 2, param2] # Modify parameters
  end

  before :my_method do |param1, param2|
    puts "Second before hook: #{param1}, #{param2}"
    [param1, param2 + 1] # Further modify parameters
  end

  # Multiple after hooks
  after :my_method do |result|
    puts "First after hook, result: #{result}"
  end

  after :my_method do |result|
    puts "Second after hook, result: #{result}"
  end

  # Multiple around hooks - nested execution
  around :my_method do |*args, &block|
    puts "First around - before"
    result = block.call
    puts "First around - after"
    result
  end
...
puts "\n=== Hook Management ==="
# Add hooks to private method
MyClass.before :private_method do |msg|
  puts "Before private method"
  msg
end

puts "Hooks for my_method: #{MyClass.hooks_for(:my_method).keys}"

# Example of removing hooks would require storing hook references:
# hook_ref = proc { puts "Removable hook" }
# MyClass.before :my_method, &hook_ref
# MyClass.remove_hook :my_method, :before, &hook_ref
