...
class RethinkdbHelper
  ...
  #         r.db('my_db').table('my_table')
  #          .filter(r.row('field_one').match("(?i)value_one"))
  #          .filter(r.row('field_two').match("(?i)value_two"))
  #
  # returns an enumerable cursor into the database table for
  ...
  def search(params={})
    ...
    @table.filter{|doc| doc[field_name].match(search_regex)}
          .run

    # a_string = "@table"
    # params.each_pair do |field_name, search_regex|
    #   a_string += ".filter{|doc| doc['#{field_name}'].match('#{search_regex}')}"
    # end
    # a_string += '.run'
    # eval a_string
